...
public class OrderService {
    ...
    /* 설명. OrderDTP에 담겨있을 내용
            : Service계층부터 개발할 때는 사용자가 입력한 값들이 어떻게 DTO 또는 Map으로 묶여서
            Service로 넘어올 지 충분히 고민한 후 매개변수를 작성하고 개발한다.
            현재의 겨웅 사용자가 고른 메뉴를 각각의 코드 번호와 고른 메뉴 갯수, 그리고 서버의 현재 시간이
            담긴 채로 넘어왔다는 생각을 가지고 개발해나가자.
     */
    /* 메모. @Transactional: 해당 메소드가 문제 없이 실행되면 자동 commit */
...
//        System.out.println("menuCode: " + menuCode);
        ...
        /* 설명 4. 1부터 3까지를 하면 tbl_order 테이블에 insert가 가능하다. */
        /* 설명 4-1. insert를 하기 위해 테이블과 매칭되는 Entity로 옮겨 담는다.(DTO -> Entity) */
        /* 메모. 사용자가 입력한 값(orderInfo)을 dto에 담아 엔티티에 옮겨 담는 코드(dto는 테이블과 1:1로 연결되어있음)
        *   입력된 값을 조회할 때에도 엔티티에 담긴 값을 dto에 옮겨와서 출력하게 됨 */
...
//        );
    ...
    /* 설명.격리레벨 (동시성) => 프로젝트마다 격리 레벨을 다르게 함
     ...
     *                    트랜젝션이 지속되는 동안에는 다른 트랜젝션이 해당 필드를 변경할 수 없다.  (다른 로우 추가는 막을 수 없다.)
